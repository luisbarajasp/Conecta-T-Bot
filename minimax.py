    ...
    def checkForStreak(self, board, player, streak):
        ...
        for r in range(6):
            for c in range(7):
                ...
                if board[r][c] == player:
                    if (streak == 4):
                        count += self.checkAnyT(r, c, player, board)
                    else:
                        # check if a vertical streak starts at (i, j)
                        count += self.verticalStreak(r, c, board, streak)

                        # check if a horizontal four-in-a-row starts at (i, j)
                        count += self.horizontalStreak(r, c, board, streak)

                        # check if a diagonal (either way) four-in-a-row starts at (i, j)
                        count += self.diagonalCheck(r, c, board, streak)
